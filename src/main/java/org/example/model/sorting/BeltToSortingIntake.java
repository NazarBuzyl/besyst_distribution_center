...
public class BeltToSortingIntake extends Thread {
    ...
    @Override
    public void run() {
        ConveyorBelt belt = inBelts.getBelts().get(beltIndex - 1);

        try {
            while (running && !isInterrupted()) {
                // Blockiert bis an Ende des Bandes ein Paket angekommen ist
                Package p = belt.pickPackageAndReturn(employeeId);
